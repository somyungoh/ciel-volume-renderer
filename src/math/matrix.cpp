...
const Matrix operator*(const double w, const Matrix &v) { return v * w; }
...
double Matrix::trace() const { return (m[0][0] + m[1][1] + m[2][2]); }
...
bool Matrix::operator>(const Matrix &v) const { return (det() > v.det()); }
bool Matrix::operator>=(const Matrix &v) const { return (det() >= v.det()); }
bool Matrix::operator<(const Matrix &v) const { return (det() < v.det()); }
bool Matrix::operator<=(const Matrix &v) const { return (det() <= v.det()); }
...
double Matrix::cofactor(int i, int j) const
{
    ...
    return pow(-1.0, (i + j)) *
           (small[0][0] * small[1][1] - small[0][1] * small[1][0]);
}
