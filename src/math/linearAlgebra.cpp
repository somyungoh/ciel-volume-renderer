...
void dotProduct(const Vector &v1, const Vector &v2, Matrix &m)
{
    m.Set(0, 0, v1(0) * v2(0));
    m.Set(0, 1, v1(0) * v2(1));
    m.Set(0, 2, v1(0) * v2(2));
...
const Matrix rotation(const Vector &axis, const double angle)
{
    ...
    Matrix       op;
    dotProduct(ax, ax, op);
    Matrix result = unitMatrix() * cosa + op * (1.0 - cosa) +
...
double       det(const Matrix &m) { return m.det(); }
double       trace(const Matrix &m) { return m.trace(); }
